...
## 十三、最近这轮 merge/build 实现，已经收敛到一个更平衡的版本

真正反复拉扯的，不是 `scan` 或 `export`，而是最后这两步：
...
到最近这一轮，Go 版实现基本收敛到了现在这条路：

- `merge` 继续产出按 `instId/field` 去重、严格时间有序的 `_merged_go/*.bin.zst`
- `build` 按 `instId` 重放这些 merged 字段流，再重新拼成最终行
- `action` / `ts` / `checksum` 写成真正的整数列
- `asks` / `bids` 不再直接写整段 JSON 文本，而是写成 packed binary
- `instId` 不再按行重复写进 Parquet，而是进入 parquet metadata 和 `summary.json`
- 如果只想从 `_merged_go` 继续推进 `build`，现在也可以在不重新依赖 source roots 的前提下单独跑，但前提是先检查 merge 状态完整

这里面最核心的变化，是盘口最终 schema 的取舍。

如果完全保留原始 JSON，写入路径最快，但最终文件会明显更大。  
如果把盘口彻底摊平成 `asksPx/asksSz/...`、`bidsPx/bidsSz/...` 这样的 full-depth list 列，虽然列式语义更漂亮，但 Parquet 的 nested/repeated 开销又会重新回来，写入也会更慢。

最后选中的方案是一个折中：

- 每个 dataset 先看前 `10` 行盘口
- 统计价格和数量里观测到的小数位
- 再额外留 `2` 位余量
- scale 只写进 parquet metadata，不在每行重复
- 每个盘口 level 编码成固定布局：
  - `px:i64le`
  - `sz:i64le`
  - `liq:i64le`
  - `num_orders:i64le`
- 如果某一行 payload 不能安全编码成这套格式，就回退到 `asksRaw` / `bidsRaw`

这意味着当前 `build` 的最终输出，本质上是：

- 标量字段尽量写成真正的数值列
- 深度簿主体仍然保留在单列 binary 中
- scale 和 `instId` 放进 dataset 级 metadata
- 极少数异常行仍然保留原始文本 fallback

这样做的直接结果是：它没有 JSON 那么快，但也没有走到最慢的 nested list 路线上。

从这轮实测看，这个版本已经比较像“平衡点”：

- `3 workers` 时大约 `15k rows/s`，整体 CPU 利用率约 `34%`
- `6 workers` 时大约 `25k rows/s`，整体 CPU 利用率约 `66%`
- 相比直接 JSON 行格式，最终文件体积大约还能再缩小 `50%` 左右

这不是绝对最快的 schema，但已经不是“慢到不值得”的那种方案。  
对 `okex_depth` 这种最终要长期保存、但也必须现实地跑完的构建任务来说，它开始像一个可接受的工程平衡点。

同时，校验链也补得更完整了。

除了 merge 之后的 raw 清理前检查：
...
这个脚本会从 `_merged_go` 重放出 build 预期行，再去和最终 parquet 做抽样或全量比对。  
这样 merge 和 build 的“完成”就不再只是看状态文件，而是能直接做内容级确认。

## 十四、恢复环境本身也会制造污染，所以要主动管理
...
- build 完成后的 merged vs parquet 校验：
  - [python/check_okex_depth_build_against_merged.py](python/check_okex_depth_build_against_merged.py)
